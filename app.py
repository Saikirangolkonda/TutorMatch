...
def payment():
    ...
    if not booking:
        abort(404)
    return f"<h1>Pay ${booking['total_price']}</h1><form method='post' action='/process-payment'><input name='booking_id' value='{booking_id}'><input name='email'><input name='phone'><input name='payment_method'><button>Pay</button></form>"
